import { validateEmail, validatePassword, validateReenteredPassword } from '../authentication-utils';
...
describe('Authentication Utilities Tests', function() {
  ...
  test('valid password and reentered password entered, should return successful', () => {
    const validationResponse = validateReenteredPassword(VALID_PASSWORD, VALID_PASSWORD);
    expect(validationResponse).toEqual(SUCCESS);
  });

  test('empty reentered password entered, should return empty reentered password response', () => {
    const validationResponse = validateReenteredPassword(VALID_PASSWORD, EMPTY_VALUE);
  ...
  test("different reentered password entered, should return passwords don't match response", () => {
    const validationResponse = validateReenteredPassword(VALID_PASSWORD, DIFF_PASSWORD);
    expect(validationResponse).toEqual(ERROR_MISMATCH_PASSWORDS);
  });
});
